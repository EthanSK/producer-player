...
**Trigger:** Ethan production report immediately after v3.337: opening Version History showed only the current export until latest-track LUFS warmup finished
**Symptom:** The selected track's older version rows, filenames, dates, and sizes were hidden for seconds even though only their LUFS/sample-rate analysis needed to remain deferred.
**Root cause:** v3.337 gated the renderer's `loadSongVersionHistory` metadata request behind the visible latest-track LUFS queue. The domain service then compounded the delay by waiting for auto-organization before broadcasting the already-read history metadata. Once that coupling was removed, the earlier snapshot exposed a selected-panel race: the shared session cache could become ready while the panel stayed on `Loading`. The two-phase current-track pass also decoded ebur128 twice because its full-enrichment follow-up repeated the loudness pass.
**Fix:** Keep two explicit lanes. Request selected-song version-history structure immediately and broadcast its readdir/stat snapshot before auto-organization completes. After an organization move, re-read only the selected song so row action paths remain valid without eagerly loading every other song's archive, and bound revision retries. Independently keep archived measured analysis behind visible latest-track LUFS warmup at BACKGROUND priority. History rows render neutral em dashes until their later analysis arrives; no analysis is started merely to show the rows. Make the selected panel react to its exact shared-cache entry and request enrichment when its LUFS-only entry is incomplete. Make Phase 2 enrichment-only so it merges sample-peak/format/BPM data into Phase 1 LUFS without a second ebur128 decode or a dedupe race that can overwrite LUFS with nulls. Interrupted warmup attempts stay pending and retry with a capped exponential backoff instead of becoming a synthetic terminal error that opens the archived-analysis gate or an unbounded retry loop.
**Commit:** this change (target v3.338)
**Guard:** `bg-tasks-visible-tracks.spec.ts` requires all three history rows within 1.5 seconds while visible LUFS is still incomplete, requires archived LUFS cells to remain neutral, and proves no BACKGROUND analysis was queued. The multi-song domain integration test blocks auto-organization, requires the complete selected-song metadata snapshot to emit first, then performs a real archived-newer-version promotion, verifies every published action path exists, and keeps the other song's history cold. `track-switch-cache.spec.ts` covers the selected-panel/cache synchronization and asserts that all visible current rows retain ready LUFS and become fully enriched after pause.
---
...
**Fix:** Give visible top-level versions an explicit first phase: run one sequential, below-normal-priority, single-threaded ebur128-only pass (LUFS/LRA/true peak plus input duration) even during established playback, while still honoring the playback settle window. Request selected-song history metadata immediately, but do not enqueue archived measured analysis until every visible latest row has a cache result or terminal error; then full volumedetect/ffprobe enrichment fills every visible current version while paused, followed by hidden-library latest tracks. Pin active/latest and organizer chronology to version number while preferring non-`old/` exports, key the warmup effect by stable analysis strings instead of array identities, make status writes idempotent, use bounded retry for interrupted rows, render a fresh null-LUFS result as `No LUFS`, and show the inspector's current phase in its loading copy. Session-only analysis caching remains unchanged.
