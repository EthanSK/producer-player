...
import {
  ...
  TECH_INFO_LRA,
  TECH_INFO_SPECTRUM,
  TECH_INFO_MID_SIDE_SPECTRUM,
  TECH_INFO_LEVEL_METER,
  TECH_INFO_CREST_FACTOR,
  TECH_INFO_PLATFORM_NORMALIZATION,
  TECH_INFO_LEVEL_MATCH,
  TECH_INFO_LOUDNESS_HISTORY,
  TECH_INFO_WAVEFORM,
  TECH_INFO_STEREO_CORRELATION,
  TECH_INFO_VECTORSCOPE,
  TECH_INFO_TONAL_BALANCE,
  TECH_INFO_K_METERING,
  TECH_INFO_QUICK_DIAGNOSTICS,
  TECH_INFO_MASTERING_CHECKLIST,
  TECH_INFO_CREST_FACTOR_HISTORY,
  TECH_INFO_LOUDNESS_DISTRIBUTION,
  TECH_INFO_SPECTROGRAM,
  TECH_INFO_SAMPLE_PEAK,
  TECH_INFO_PEAK_SHORT_TERM,
  TECH_INFO_PEAK_MOMENTARY,
  TECH_INFO_MEAN_VOLUME,
  TECH_INFO_CLIP_COUNT,
  TECH_INFO_DC_OFFSET,
  TECH_INFO_MID_SIDE_MONITORING,
  TECH_INFO_CURRENT_LOUDNESS,
} from './technicalInfoText';
import {
  AgentChatPanel,
...
const MEASURED_ANALYSIS_QUEUE = new AnalysisQueue({
  ...
  // Item #14/v3.219 — USER_SELECTED ffmpeg jobs interrupt lower-priority
 ...
 *
 * Given one of the mastering rule IDs and the measured values that were
 * used to render it, produce a short, human-readable checklist item
 * describing the current value and the target the user should aim for.
 * Returns null when the row doesn't have enough data to generate a
 * sensible message (e.g. LUFS / True Peak weren't measured yet).
 * Timestamp is intentionally omitted — mastering findings are timeless
 * (Ethan's words: "It doesn't need to be associated with any time").
 *
 * v3.28.0 — expanded from the four legacy rows to the full rule set in
 * `masteringChecklistRules.ts`. Any rule ID is now accepted; messages for
 * the new rules are generated by evaluating the rule with the current
 * inputs and combining its label + value + message into a one-liner.
 *
 * Legacy IDs (`lufs`, `true-peak`, `dc-offset`, `clipping`) keep the
 * original, hand-tuned copy so the v3.26 Playwright spec — which
 * asserts substrings like `LUFS Integrated` and `dBTP` — still passes.
 */
export type MasteringChecklistRowId = string;
...
export interface MasteringChecklistRowInputs {
  ...
  dcOffset: number;
  clipCount: number;
  measuredAnalysis?: AudioFileAnalysis | null;
  trackAnalysis?: TrackAnalysisResult | null;
}

function buildMasteringChecklistEvaluationInput(
  inputs: MasteringChecklistRowInputs,
): MasteringChecklistEvaluationInput {
  // Prefer the richer objects when the caller supplies them; otherwise
  // synthesize a minimal `measured` / `analysis` pair from the legacy
  // four-field shape so the simple rules still render sensibly.
  if (inputs.measuredAnalysis !== undefined || inputs.trackAnalysis !== undefined) {
    return {
      measured: inputs.measuredAnalysis ?? null,
      analysis: inputs.trackAnalysis ?? null,
    };
  }
  return {
    measured: {
      ...
      integratedLufs: inputs.integratedLufs,
      loudnessRangeLufs: null,
      truePeakDbfs: inputs.truePeakDbfs,
      samplePeakDbfs: null,
      meanVolumeDbfs: null,
    ...
    analysis: {
      ...
      rmsDbfs: 0,
      crestFactorDb: 0,
      dcOffset: inputs.dcOffset,
      clipCount: inputs.clipCount,
...
export function buildMasteringChecklistItemText(
  ...
  // Preserve the hand-tuned legacy copy for the four v3.26 rows so the
  // existing Playwright spec continues to assert the same substrings.
  switch (rowId) {
    case 'lufs': {
      if (inputs.integratedLufs === null) return null;
      const lufs = inputs.integratedLufs;
      const withinRange = lufs >= -16 && lufs <= -8;
      if (withinRange) {
        return `Loudness check — currently ${lufs.toFixed(1)} LUFS Integrated, within the -16 to -8 streaming range`;
      }
      if (lufs > -8) {
        return `Reduce loudness — currently ${lufs.toFixed(1)} LUFS Integrated, target -16 to -8 LUFS for streaming normalization`;
      }
      return `Increase loudness — currently ${lufs.toFixed(1)} LUFS Integrated, target -16 to -8 LUFS for streaming normalization`;
    }
    case 'true-peak': {
    ...
    case 'clipping': {
      ...
      // v3.28 — graduated tiers. Text still reads the same "Fix clipping"
      // regardless of tier, so the promoted item keeps its actionable
      // phrasing.
      return `Fix clipping — ${inputs.clipCount} clipped sample${plural} detected in the master, reduce gain before the limiter`;
    }
    default: {
      // v3.28 — generic dispatch for the new Phase 1 rules. Runs the
      // rule against the current inputs and returns a templated
      // summary. If the rule can't evaluate (unavailable status) we
      // return null so the promotion flow silently skips.
      const rule = getMasteringChecklistRuleById(rowId);
      if (!rule) return null;
      const evaluation = rule.evaluate(buildMasteringChecklistEvaluationInput(inputs));
      if (evaluation.status === 'unavailable') return null;
      const prefix =
        evaluation.status === 'pass'
          ? `${rule.label} check`
          : evaluation.status === 'warn'
            ? `Review ${rule.label.toLowerCase()}`
            : `Fix ${rule.label.toLowerCase()}`;
      return `${prefix} — currently ${evaluation.value}, ${evaluation.message}`;
    }
  }
}
...
export function App(): JSX.Element {
  ...
  const [agentDangerouslyBypassPermissions, setAgentDangerouslyBypassPermissions] =
    useState<boolean>(false);
  // v3.145 — the user-facing background precompute pause switch was removed.
  // Keep writing the legacy unified-state field as true so old persisted
  // `agentBackgroundPrecomputeEnabled=false` values are harmless after launch.
  ...
  useEffect(() => {
    ...
    if (AudioContextConstructor) {
      try {
        ...
        // → audition processing (mid-side / EQ / solo)
        // → analyser/meters → final player volume → destination
        //
        ...
        // hearing, while remaining before the one user-facing volume slider.
        ...
        // Stereo split branch (post-preview/post-audition-processing, pre-volume)
            ...
            {selectedPlaybackVersion ? (
              <div className="analysis-overlay-grid">
                ...
                >
                  <div className="analysis-panel-header-row">
                      <div className="analysis-section-header">
                      <h4>Mid/Side Monitoring{referenceModeSuffixNode} <HelpTooltip text="Listen to just the center (Mid) or sides (Side) of your stereo mix separately. Mid = vocals, bass, kick. Side = reverb, width, panning. Useful for checking stereo balance." links={MID_SIDE_LINKS} /><TechnicalInfoPopover text={TECH_INFO_MID_SIDE_MONITORING} /></h4>
                ...
                {(() => {
                  ...
                  >
                    <div className="analysis-panel-header-row">
                      <div className="analysis-section-header">
                      <h4>Mastering Checklist <HelpTooltip text={"What this measures: An automated technical health check of your master, grouped by category. Loudness: LUFS integrated (streaming range -16..-8), loudness range (LRA), short-term vs integrated gap, and momentary peak. Peaks: true peak (target < -1 dBTP), clipped samples, inter-sample risk. Dynamics: crest factor (PLR) and over-limiting duration. Spectrum: bass and treble balance. Housekeeping: DC offset, leading/trailing silence trim, sample rate, noise floor, and truncated tail.\n\nGood values: All rows showing a pass (checkmark). Warns flag things worth checking; fails mean the master will be audibly worse on release.\n\nIf it's wrong: LUFS warning — adjust overall loudness to match streaming targets. True Peak warning — lower your limiter ceiling to -1 dBTP or use a true peak limiter. DC Offset warning — apply a high-pass filter at 10-20 Hz or use DC offset removal. Clipping failure — reduce gain into your final limiter. Spectrum/dynamics warnings point at EQ, limiter threshold, or transient treatment changes."} links={MASTERING_CHECKLIST_LINKS} /><TechnicalInfoPopover text={TECH_INFO_MASTERING_CHECKLIST} />{referenceModeSuffixNode}</h4>
                    ...
                    <div className="mastering-checklist-summary">
                      ...
                      {!isChecklistReady || !checklistAnalysis || !checklistMeasured ? (
                        // v3.28.0 — skeleton mirrors the full rule set
                        // grouped by category, matching the loaded state's
                        // row + category-label count exactly. Previous
                        // (v3.27) hard-coded 4 rows; after the Phase 1
                        // expansion to ~16 rules the pending→ready
                        // transition would shift ~200px downward if the
                        // skeleton stayed at 4. Now the skeleton uses the
                        // same `groupMasteringChecklistRules` grouping as
                        // the loaded branch — any future rule addition is
                        // automatically mirrored on both sides.
                        <>
                          {groupMasteringChecklistRules().map((group) => {
                            if (group.rules.length === 0) return null;
                            return (
                              <div
                                key={group.category}
                                className="mastering-checklist-category"
                                data-testid={`mastering-checklist-category-${group.category.toLowerCase()}`}
                              >
                                <div className="mastering-checklist-category-label">
                                  {group.category}
                                </div>
                                {group.rules.map((rule) => (
                                  <div
                                    ...
                                    data-testid={`mastering-checklist-row-skeleton-${rule.id}`}
                                    aria-busy="true"
                                  >
                                    ...
                                    {/*
                                     * v3.27.0 — Codex review follow-up:
                                     * the loaded rows render `+ Add to
                                     * checklist` on the right. Render
                                     * an invisible placeholder so the
                                     * row box matches the loaded state
                                     * exactly.
                                     */}
                                    {!isRefMode ? (
                                      <span
                        ...
                        // v3.28.0 — drive the rendered rows from
                        // `MASTERING_CHECKLIST_RULES`. Rules are grouped by
                        // category so the panel stays readable as the rule
                        // count grows past 4. Each row resolves its status
                        // (pass/warn/fail/unavailable) via the rule's
                        // `evaluate` function — same source of truth as
                        // `buildMasteringChecklistItemText` and the unit
                        // tests.
                        const evaluationInput: MasteringChecklistEvaluationInput = {
                          measured: checklistMeasured,
                        ...
                        const masteringInputs: MasteringChecklistRowInputs = {
                          ...
                          measuredAnalysis: checklistMeasured,
                          trackAnalysis: checklistAnalysis,
                        };
                            ...
                            {groups.map((group) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {group.rules.map((rule) => {
                                    const evaluation = rule.evaluate(evaluationInput);
