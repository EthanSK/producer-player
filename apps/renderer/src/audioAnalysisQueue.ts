...
// rejected. v3.219 makes USER_SELECTED a real interrupt: lower-priority
...
interface QueuedTask<T> {
  ...
  preempted: boolean;
  /**
   * v3.219 — true once preemption has already freed this run's queue slot.
  ...
  /**
   * v3.219 — closure installed by startTask so preemption can synchronously
...
export class AnalysisQueue {
  ...
  private requeuePreemptedTask(task: QueuedTask<unknown>): void {
    // Clone rather than reusing the still-settling task object. v3.219 can
  ...
  /**
   * v3.219 — Preempt all running NEIGHBOR/BG tasks that are cancellable as
