...
export const TECH_INFO_MASTERING_CHECKLIST =
  `LUFS and True Peak checks use FFmpeg-measured (authoritative) values. DC Offset and Clip Count checks use the Web Audio full-file decode (renderer-side analysis). LUFS passes if -16 to -6, True Peak passes if < -1 dBTP, DC Offset passes if mean sample value <= 0.001 (0.1%), Clipping passes if zero samples at digital ceiling. All thresholds are industry conventions, not hard rules — passing all four means your master is technically clean for distribution.`;
