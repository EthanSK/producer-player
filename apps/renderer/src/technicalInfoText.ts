...
export const TECH_INFO_LOUDNESS_HISTORY =
  `The curve plots per-frame loudness from the full-file Web Audio decode. The track is divided into individual 250 ms frames; each frame's RMS amplitude is converted to dBFS and plotted directly (not a rolling window). Red spans and red curve sections mark time ranges whose waveform peak buckets hit the 0 dBFS digital ceiling, matching the same Web Audio sample-clip detection used by Clip Count. This is an RMS-based approximation — not true LUFS (no K-weighting or gating). The dashed horizontal line is the FFmpeg authoritative integrated LUFS for reference. Use this graph for section-by-section comparison, not for final delivery numbers.`;
